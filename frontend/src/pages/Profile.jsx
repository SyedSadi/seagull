      ...
      <div className="mt-6">
        ...
        {loading ? (
          ...
          <ul className="space-y-3">
            {courses.map((course) => (
              <li key={course.id} className="flex justify-between items-center p-3 border rounded-lg">
                <div>
                  <h3 className="text-lg font-medium">{course.title}</h3>
                  <p className="text-gray-500">{course.subject}</p>
                  {user.role === "instructor" && (
                    <p className="text-sm text-gray-600">Rating: {course.ratings.toFixed(1)} ⭐</p>
                  )}
                </div>
                <Link to={`/courseContents/${course.id}`} className="btn btn-primary">
                  {user.role === "student" ? "Go to Course" : "View Course"} <FaArrowRight />
                </Link>
              </li>
            ))}
          </ul>
