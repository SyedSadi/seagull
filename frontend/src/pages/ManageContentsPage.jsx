...
const ManageContentsPage = () => {
	...
	const handleSubmit = async (e) => {
		...
		const validationErrors = contents
			.map((content, index) => {
				const errors = [];
				...
				if (
					["text", "article"].includes(content.content_type) &&
		...
		try {
			...
			const updatePromises = contents.map((content) =>
				updateContentById(content.id, content)
			);
	...
	return (
		<div>
			<h2 className="text-2xl text-center font-semibold m-6">
				Manage Contents
			</h2>
			{contents.map((content, index) => (
				<div
					key={content.id || index}
					className="max-w-lg mx-auto p-6 shadow-lg bg-base-100 rounded-lg"
				>
